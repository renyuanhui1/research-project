...
CSV_PATH = PROJECT_ROOT / "data/run-10-trajectory.csv"

NED_START = np.array([-423.10, -460.70, -11.80])

# COLMAP 单位 → 米。COLMAP 坐标系未必与 NED 正交对齐，
# 所以三个轴各自独立缩放。
# 用上一次飞行末端位置反推：
#   北（来自 -cx）：期望 16.8 m / 实测 7.11 → ≈ 2.36
#   东（来自  cz）：期望 10.6 m / 实测 8.85 → ≈ 1.20
#   下（来自 -cy）：垂直方向暂保持 1.6，先看水平
SCALE_NORTH = 2.36
SCALE_EAST = 1.20
SCALE_DOWN = 1.6
...
def colmap_to_ned_offset(colmap_delta):
    ...
    Each axis has its own scale because COLMAP's coordinate frame
    isn't necessarily aligned with NED.
    """
    cx, cy, cz = colmap_delta
    n_north = -cx * SCALE_NORTH
    n_east = cz * SCALE_EAST
    n_down = -cy * SCALE_DOWN
...
async def main():
    ...
    try:
        ...
        projectairsim_log().info(
            f"Flying {len(path)} waypoints, "
            f"scale=(N={SCALE_NORTH}, E={SCALE_EAST}, D={SCALE_DOWN})"
        )
