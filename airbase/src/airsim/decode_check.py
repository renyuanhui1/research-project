...
def parse_args():
    ...
    p.add_argument("--altitude", type=float, default=30.0, help="飞到目标点上方多少米再采集")
    p.add_argument("--forward", type=float, default=3.0, help="采集时前进速度 m/s")
    p.add_argument("--target-x", type=float, default=None, help="飞到的目标点 NED north(米);默认原地上升")
    p.add_argument("--target-y", type=float, default=None, help="飞到的目标点 NED east(米)")
    return p.parse_args()
...
async def main(args):
    ...
    try:
        ...
        # 起飞后飞到目标点(NED)上方 args.altitude 米再采集; 未给 target 则原地上升
        pos = drone.get_ground_truth_kinematics()["pose"]["position"]
        tn = float(pos["x"]) if args.target_x is None else args.target_x
        te = float(pos["y"]) if args.target_y is None else args.target_y
        print(f"飞到 N={tn:.1f}, E={te:.1f} 上方 {args.altitude:.0f}m ...")
        await (await drone.move_to_position_async(
            north=tn, east=te, down=-args.altitude, velocity=10.0))

        await wait_first_frame()
        print(f"已到目标点 {args.altitude:.0f}m，开始存图（共 {args.frames} 张，每 {args.interval}s 一张）")

        # 在目标高度缓慢前进采集(保持高度, 画面有变化)
        total = max(args.frames * args.interval + 1.0, 3.0)
        move_task = await drone.move_by_velocity_async(
            v_north=args.forward, v_east=0.0, v_down=0.0, duration=total
        )
